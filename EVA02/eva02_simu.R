...
Cosmetico <- read.transactions("D:/ULIMA/2024-2/Analitica Predictiva de Datos/Evaluaciones/SIMULACION/EVA02/Cosmetic.txt", header = F, format = 'basket', sep = ',')

View(Cosmetico)
# Ver la data sparse
Cosmetico@data
# Visualizar la matriz de transacciones (p. ej. para las 5 primeras transacciones)
image(Cosmetico[1:20])
...
Cosmeticorules <- apriori(Cosmetico, parameter = list(support =0.1,
...
library(arulesViz)
plot(Cosmeticorules)
plot(Cosmeticorules,method="graph",control=list(alpha=1))

subrules <- head(sort(Cosmeticorules, by="lift"), 10)
plot(subrules)
...
inspect(sort(Cosmeticorules, by = "lift")[1:5])
...
bayasrules <- subset(Cosmeticorules, items %in% "bayas")
inspect(bayasrules)

bayas_yogurtrules <- subset(Cosmeticorules, items %in%c("bayas", "yogurt"))
inspect(bayas_yogurtrules)

bayasrules <- subset(Cosmeticorules, items %ain%c("bayas", "yogurt"))
inspect(bayasrules)
...
reglas_galletas1 <-apriori(Cosmetico, parameter = list(support =0.1,
                                                          confidence = 0.5, minlen = 2),
                           appearance = list(rhs = "Mascara"))

inspect(sort(reglas_galletas1, by = "lift")[1:5])

reglas_galletas2 <-apriori(Cosmetico, parameter = list(support =0.006,
...
write(Cosmeticorules, file = "Cosmeticorules.csv",
      sep = ",", quote = TRUE, row.names = FALSE)

# Convertir reglas en dataframe
Cosmeticorules_df <- as(Cosmeticorules, "data.frame")
head(Cosmeticorules_df)
...
#PREGUNTA1
summary(Cosmetico)
#Calcular numero de productos comprados (filax*columnas*density)
957*14*0.3273623
...
#PREGUNTA4
itemFrequencyPlot(Cosmetico, support = 0.1)
itemFrequencyPlot(Cosmetico, topN = 20)
...
reglas_mascara <-apriori(Cosmetico, parameter = list(support =0.1,
                                                       confidence = 0.5, minlen = 2),
                           appearance = list(lhs = "Mascara"))
